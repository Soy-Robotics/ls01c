...
static int Uart_parameter(unsigned char *data, double *angle, double *dist, int len)
{
	...
	//printf("len = %d, data[3610] = %02x\n", len, data[3610]);
	if (data[0] == 0xA5 && data[6] == 0x81 &&data[len-4] == 0xaa &&data[len-1] == 0xdd){
		tmp = data+7;
		g_speed = data[1]/15.0;
...
int io_driver::StopScan(void)
{
	int ret;
       ...
       char buf2[7] = {0xa5, 0x25, 0xe1, 0xAA, 0xBB, 0xCC, 0xDD};
	ret = write(m_dFd, buf, 7);
    usleep(3000);
    ret = write(m_dFd, buf2, 7);
	return  ret;
}
